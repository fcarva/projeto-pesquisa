...
por conta própria \citep{coase1960} --- e desloca a pergunta para \emph{quem
detém o direito}: o de pulverizar, ou o de não ser pulverizado.
...
equivalentes em bem-estar esperado} \citep{weitzman1974}. Qual deles domina
depende das \textbf{inclinações relativas} das curvas de benefício e custo
marginais.
...
\paragraph{E é o que ocorreu, o que obriga a ler esta subseção com a
§\ref{sub:ancora} ao lado.}
A curva foi estimada e seu agregado não se mostrou interpretável, de modo que a
âncora prometida no parágrafo acima não se materializou nesta versão. O
argumento teórico permanece válido --- Weitzman de fato decide pela inclinação
---; o que muda é de onde virá a informação sobre ela, e a conclusão adota a
rota da \emph{inversão}: perguntar qual inclinação faria o ranking virar, em vez
de estimá-la.

\paragraph{Uma imprecisão desta subseção, corrigida na conclusão.}
Dizer que o Ensaio 2 ``consome a derivada'' identifica o objeto errado por uma
ordem. A regra compara a \emph{inclinação} do benefício marginal com a do custo
marginal, e o benefício marginal já é a derivada da dose-resposta: o que
Weitzman consome é \(ACR'\), a derivada segunda. Ter a curva inteira --- e não
um ponto --- é o que tornaria esse objeto acessível, e é a justificativa correta
para o alvo escolhido. A §\ref{sub:ancora} desenvolve o mapeamento.
...
a regulação por quantidade gera maior lucro ao setor regulado
\citep{helfand1995} --- resultado com consequências distributivas, e não apenas
de eficiência. A observação importa porque a proibição cearense foi contestada
judicialmente pelo lobby do setor, e não por consumidores.
...
da restrição, e em que proporção \citep{lichtenberg1988,fernandezcornejo1998}.
A segunda é a comparação empírica de instrumentos, em que há registro de
cenários nos quais cotas superam taxas
\citep{skevas2012} --- isto é, o resultado de Weitzman aplicado, e não apenas
invocado.
...
instrumento \citep{roth2002engineer}. Tratá-lo apenas como exercício de
maximização de bem-estar esperado, porém, deixa fora do quadro uma camada que
este caso torna difícil de ignorar.
