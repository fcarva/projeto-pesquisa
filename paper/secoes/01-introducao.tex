...
registradas com antecedência. \textbf{O que se defende aqui é o desenho, não um
efeito.}
\end{quote}

\paragraph{E há estimativa neste documento, o que exige uma explicação.}
Versão anterior desta seção afirmava que não haveria resultado estimado, e a
afirmação deixou de ser verdadeira: a estimação foi levada até o fim, e a
§\ref{sec:diagnostico} a reporta. A seção \textbf{não se chama
``Resultados''}, e o título não é eufemismo. O que ela mostra é que a análise de
poder \emph{ex ante} desta dissertação estava certa --- previa efeito mínimo
detectável de 33,4 g, e a meia-largura realizada foi de 30,3 g. Isso é
informação sobre o \emph{desenho}, que é o que esta etapa defende.

Nenhum número daquela seção é apresentado como achado. A pré-especificação,
fechada e versionada antes de qualquer coeficiente existir, comprometeu que
resultado abaixo do efeito mínimo detectável se escreve como \textbf{limite
...
estimando e o que o desenho lhe opõe. A §\ref{sec:dados} documenta a aquisição
de cada fonte --- incluindo as que foram obtidas, inspecionadas e
\emph{descartadas}, porque o defeito de cada uma é informação sobre o desenho ---
e as descritivas do painel. A §\ref{sec:diagnostico} traz o diagnóstico de poder
e o limite superior, sob o enquadramento explicado acima. A
§\ref{sec:conclusao} conclui, e encara de frente a consequência que o
diagnóstico tem para o segundo ensaio.
